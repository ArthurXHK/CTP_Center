...
class Trader(TraderApi):
    ...
    def Connect(self):
        self.Create("trader")
        self.RegisterFront(self.tdserver)
    ...
    def OnFrontConnected(self):
        print self.tradername, ' is connected'
        self.Login(self.brokerid, self.investorid, self.password)

    def Login(self, brokerid, investorid, password):
        req = ApiStruct.ReqUserLogin(BrokerID=brokerid, UserID=investorid, Password=password)
        self.requestid+=1
...
def main():
    user = Trader(tradername = 'my', brokerid="1019",investorid="00000001",password="123456",tdserver="tcp://183.129.188.37:41205")
    user.Connect()
    
    user.QryMarketData('IF1501')
    print user.GetTradingDay()
    user.Join()

...
{
    PrintLog(string(__FUNCTION__) + GetPortMsg(pTraderApi) + ": " + pRspInfo->ErrorMsg, "error");
}
...
{
    PrintLog(string(__FUNCTION__) + GetPortMsg(pApi) + ": " + pRspInfo->ErrorMsg, "error");
}
...
{
    if(pOrder->SessionID == TD_GetSessionID(pTraderApi))
        PrintLog(string(__FUNCTION__) + GetPortMsg(pTraderApi) +
                string("报单号") + pOrder->OrderRef + string("报单状态: ") + pOrder->OrderStatus);
}
